...
- floodoh.py:
    ```
    kubectl exec -n ddos {{ item.pod }} -- env https_proxy="http://{{ internet_ip }}:3128" https_proxy="http://{{ internet_ip }}:3128" ddosserver={{ ddosserver_ip }} python3 cne-DoH-master/floodoh.py <n_connect> <set_qname> <set_qtype> <doh_url>
    ```
